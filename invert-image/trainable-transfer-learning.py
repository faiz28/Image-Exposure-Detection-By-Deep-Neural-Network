...
file = open(DIR+'trainable-tf.txt',"w")
def main():
...
def training(trainX, trainY, modelPath):
    ...
    history = model.fit(trainX, trainY, epochs=150, batch_size=128, validation_split=0.2, callbacks=callbackList)
...
def plot_loss_acc(history):
    ...
    plt.legend()
    plt.savefig(DIR + 'trainable-tf-training_validation_accuracy.png')
    plt.close()
    ...
    plt.savefig(DIR + 'trainable-tf-training_validation_loss.png')
    plt.close()
