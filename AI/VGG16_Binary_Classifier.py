...
def testing(testX, testY, modelPath):
	...
	predictedClass = ['Jasmine' if a < 0.5 else 'WaterLilly' for a in predictions] 
...
def training(trainX, trainY, modelPath, imgH, imgW):
	...
	history = model.fit(trainX, trainY, epochs = 100, validation_split = 0.2, callbacks = callbackList)
